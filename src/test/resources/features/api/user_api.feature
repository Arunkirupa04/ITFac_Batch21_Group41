...
Feature: User API Functionality
  ...
  Scenario: Verify 403 Forbidden when user attempts to create a sale
    When user sends a POST request to "/api/sales/plant/1" with body:
      """
      {
        "quantity": 1
      }
  ...
  Scenario: Verify user can view a specific sale
    ...
    When user sends a GET request to "/api/sales/1"
    Then the response status code should be 200

  @TC_API_USER_21
  Scenario: Verify 403 Forbidden when user attempts to delete a sale
    Given a sale exists with ID 1
    When user sends a DELETE request to "/api/sales/1"
    Then the response status code should be 403
