    ...
    public void admin_attempts_to_sell_of_plant_with_stock(int quantity, String plantName, int stock) {
        ...
        try {
            ...
            if (!found) {
                ...
                String[] options = sellPlantPage.getAllPlantOptions();
                String firstValidPlant = null;
                for (String opt : options) {
                    if (!opt.toLowerCase().contains("select") && !opt.trim().isEmpty()) {
                        firstValidPlant = opt;
                        break;
                    }
                }

                if (firstValidPlant != null) {
                    System.out.println("Falling back to first available plant: " + firstValidPlant);
                    sellPlantPage.selectPlant(firstValidPlant);
                    selectedPlantName = firstValidPlant.split(" \\(")[0].trim();
                } else {
                    Assert.fail("No valid plants available in dropdown to sell!");
                }
            }
        }

        // Smart Quantity Logic
        int actualStock = sellPlantPage.extractStockFromOption(selectedPlantName);
